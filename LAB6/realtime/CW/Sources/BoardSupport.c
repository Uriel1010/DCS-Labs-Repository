...
void InitGPIO()
{
	...
	PORTE_PCR20 = PORT_PCR_MUX(0); // analog-potentiometer (ADC)
	...
	// Key PAD + BUzzer setup
		PORTA_PCR1 =  PORT_PCR_MUX(3);  // Buzzer set as PWM(TPM2, channel0)
		PORTA_PCR2 =  PBsArrPortSel+PBsArrIntEdgeSel(PULL_UP);	// keypad interrupt
		GPIOA_PDDR = 0x01;
		GPIOA_PDOR = 0x00;
		...
		//PORTB_PCR0 = PORT_PCR_MUX(0); // analog-potentiometer (ADC)
		PORTB_PCR1 = PORT_PCR_MUX(1); // GPIO-Blue
		PORTB_PCR2 = PORT_PCR_MUX(1); // GPIO-Green
		PORTB_PCR3 = PORT_PCR_MUX(1); // GPIO-Red
		GPIOB_PDDR = 0x0E;			  //  set port B0 as Input and B1,B2,B3 as OUTPUT
		GPIOB_PDOR = 0x00;
