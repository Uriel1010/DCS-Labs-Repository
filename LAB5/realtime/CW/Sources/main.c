	...
	while(1){
		switch(state){
		  ...
		  case state4:
			  ...
			  break;
		   case state5:
			   offset = sizeof(str)/(2*sizeof(str[0]));
			   RTfirsthalf(&str,&strMirror,offset);
			   RTsecondhalf(&str,&strMirror,offset);
			   state = state0;
			   break;
		   case state6:
			   return 0;
...
void RTsecondhalf(sadd,dadd,offset){
	DMA_SAR3 = sadd+offset;
	DMA_DAR3 = dadd;
	//DMA_DCR2 = 0x12000;
	//DMA_DCR2 |= 0x480000;
	//DMA_DCR2 |=0x40000000;
	DMA_DCR3 = 0x405A0000;//DCR 8bits and source&dest inc
	//DMA_DCR2 |= 0x20000000;
	DMA_DSR_BCR3=offset;//sizeof(song1)/sizeof(song1[0]);
	DMA_DCR3|=0x80000000;
	DMA_DCR3 |= 0x10000;
					
}

void RTfirsthalf(sadd,dadd,offset){
	DMA_SAR2 = sadd;
	DMA_DAR2 = dadd+offset;
	//DMA_DCR2 = 0x12000;
	//DMA_DCR2 |= 0x480000;
	//DMA_DCR2 |=0x40000000;
	DMA_DCR2 = 0x405A0000;//DCR 8bits and source&dest inc
	//DMA_DCR2 |= 0x20000000;
	DMA_DSR_BCR2=offset; //sizeof(song1)/sizeof(song1[0]);
	DMA_DCR2|=0x80000000;
	DMA_DCR2 |= 0x10000;
